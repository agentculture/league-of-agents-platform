...
type: command
description: >
  Cross-repo + mesh communication from league-of-agents-platform: file tracked GitHub issues
  ...
  the next step lives outside league-of-agents-platform (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- league-of-agents-platform (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-league-of-agents-platform
