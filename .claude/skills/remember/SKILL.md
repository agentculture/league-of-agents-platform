...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope league-of-agents-platform
...
scope:** the default personal scope is **private** (`--scope league-of-agents-platform
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `league-of-agents-platform`). Private records are served only to a recall in the same
