"""Unified CLI entry point for league-of-agents-platform.

The agent-first global verbs (``whoami``, ``learn``, ``explain``, ``overview``,
``doctor``) are registered here under :mod:`league_site.cli._commands`,
alongside the ``cli`` noun group. Future noun groups register via their own
``register()`` functions following the same pattern.

Error propagation contract
--------------------------
Every handler raises :class:`league_site.cli._errors.CliError` on
failure; ``main()`` catches it via :func:`_dispatch` and routes through
:mod:`league_site.cli._output`. Unknown exceptions are wrapped into a
...
from league_site.cli._output import emit_error

_ISSUES_URL = "https://github.com/agentculture/league-of-agents-platform/issues"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = _CliArgumentParser(
        prog="league-of-agents-platform",
        description="league-of-agents-platform — a clonable template for AgentCulture mesh agents.",
    )
    parser.add_argument(
        "--version",
        action="version",
