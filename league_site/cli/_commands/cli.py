"""``league-of-agents-platform cli`` — noun grouping CLI-surface introspection.
...
from league_site.cli._commands.overview import cli_sections, emit_overview
...
def _no_verb(args: argparse.Namespace) -> int:
    # `league-of-agents-platform cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'league-of-agents-platform cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe the league-of-agents-platform CLI surface.")
