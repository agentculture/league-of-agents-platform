"""Smoke tests for the league-of-agents-platform CLI entry point and its verbs."""
...
from league_site.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: league-of-agents-platform" in capsys.readouterr().out
...
def test_whoami_text(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "nick: league-of-agents-platform" in out
...
def test_whoami_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["nick"] == "league-of-agents-platform"
...
def test_learn_text(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    out = capsys.readouterr().out
    assert len(out) >= 200
    assert "league-of-agents-platform" in out
...
def test_learn_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["tool"] == "league-of-agents-platform"
    assert payload["version"] == __version__
...
def test_explain_root(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain"])
    assert rc == 0
    assert "# league-of-agents-platform" in capsys.readouterr().out


def test_explain_self(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain", "league-of-agents-platform"])
    assert rc == 0
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "league-of-agents-platform whoami" in payload["markdown"]
