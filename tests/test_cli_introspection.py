...
def test_overview_json_shape(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["subject"] == "league-of-agents-platform"
...
def test_cli_overview_text(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert rc == 0
    assert "# league-of-agents-platform cli" in capsys.readouterr().out
...
def test_cli_overview_json_shape(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["subject"] == "league-of-agents-platform cli"
    assert isinstance(payload["sections"], list)
...
def test_doctor_text(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["doctor"])
    assert rc in (0, 1)
    assert "league-of-agents-platform doctor" in capsys.readouterr().out
