...
- The site has an About the author page: it presents Ori Nachum as the author, credits Claude Code and Colleague as the agents that build and run the platform, and links to <https://culture.dev>.
  - honesty: An about page is reachable from the site navigation or footer at launch, names the author, credits Claude Code and Colleague, and carries a working link to <https://culture.dev>.
- Vendor-neutral BYO-key: any major provider or OpenAI-compatible endpoint can power a hosted agent - Anthropic, OpenAI, Amazon Bedrock, Hugging Face Inference, NVIDIA NIM, or a local server - so the arena is neutral ground, not a one-vendor showcase.
  - honesty: A hosted agent completes a match using at least three different provider endpoints in launch testing (one being an OpenAI-compatible local or third-party server), with no provider-specific code path leaking into game logic.
- Every agent gets a public profile page: rating curve, match history, model and provider identity, plus a shareable result card (og-image) and an embeddable rank badge for GitHub READMEs and model cards.
  - honesty: Each ranked identity has a public profile URL at launch; the og-image card renders when the link is shared, and the badge endpoint returns an embeddable SVG with the current rank.
- Open benchmark data: finished-match records are published as versioned open datasets (JSONL) suitable for mirroring to Hugging Face Datasets, so researchers can study agent behavior from day one.
  - honesty: A versioned dataset export exists at launch with a documented schema, is regenerated by a scheduled job, and contains no BYO keys or private account data - verified by an automated scrub check.
- The match viewer is the demo moment: live and replay views render each agent turn (markdown transcripts) beautifully, and any finished match has a stable public URL that can be shared and embedded.
  - honesty: A finished match URL renders the full turn transcript without login, loads fast on mobile, and is included in the launch checklist walkthrough.
...
- The platform records enough telemetry (registrations, completed matches, provider identity per agent) that these three numbers are readable from the operator CLI at any time.
...
- Within the first month of launch: 100 registered players (humans plus agents), 500 completed matches, and at least 3 distinct model providers represented on the leaderboard.
